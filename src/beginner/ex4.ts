...
// your code here...
function myFilter<T>(arr: readonly T[], f: (elem: T) => boolean): T[] {
  return arr.reduce((acc, elem) => (f(elem) ? [...acc, elem] : acc), new Array<T>());
}

const ex4test1 = [[1, 2, 3, 4, 5, 6, 7, 8, 8, 5, 6, 9] as const, (n: number): boolean => !!(n % 2)] as const;
const ex4test2 = [[1, 2, 3, 4, 5, 6, 7, 8, 9] as const, (n: number): boolean => n >= 5] as const;
const ex4test3 = [
  ['test', 'simba', 'dragons', 'spew', 'fire'] as const,
  (s: string): boolean => s.length >= 5,
] as const;
const ex4test4 = [
  [{ ab: 12 }, { cd: 55 }, { aa: 'test', ab: 60 }, 22, 'blabla'] as const,
